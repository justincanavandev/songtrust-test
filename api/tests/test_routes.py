    ...
    with app.app_context():
        todos = [
            ToDo(title='First Todo', completed=False,
                 description="go to gym", priority=1),
            ToDo(title='Second Todo', completed=True,
                 description="get groceries", priority=2),
            ToDo(title='Third Todo', completed=True,
                 description="finish coding test", priority=3)
        ]
...
def test_create_todo_success(client):
    response = client.post(
        '/todos', json={'title': 'Test Todo', 'completed': False, "description": "do it today", "priority": 1})
...
def test_update_todo_success(client):
    response = client.post('/todos', json={'title': 'Update Test Todo',
                           "completed": False, "description": "update", "priority": 3})
    assert response.status_code == 201
